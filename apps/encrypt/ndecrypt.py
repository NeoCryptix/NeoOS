def main():
  ...
  input1 = input ('Enter value to decrypt. (spaces allowed)\n' + TGREEN + '>> ' + TWHITE )

  os.system('cls')

  val = input1.replace("-","1").replace("[","2").replace("{","3").replace("_","4").replace("/","5").replace("\\","6").replace("|","7").replace("}","8").replace("<","9").replace("]","0")
  ...
  print ('Your decrypted value is... ' + val)
